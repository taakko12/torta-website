          ...
          <table className="w-full">
            ...
            <tbody>
              {filteredLinks.length === 0 ? (
                <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-[#7878a8]">{q ? 'No matches.' : 'No links yet.'}</td></tr>
                    ...
                    <td className="px-4 py-2.5">
                      {isFirstForUser ? (
                        <>
                          ...
                          <span className="text-xs text-[#7878a8] block">{l.discord_id}</span>
                        </>
                      ) : (
                    ...
                    <td className="px-4 py-2.5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {isEditing ? (
                          ...
                          <>
                            {!l.primary_rsn && (
                              <button onClick={() => setPrimary(l.discord_id, l.rsn)} className="text-xs text-[#7878a8] hover:text-[#c89b3c] px-2 py-1 rounded border border-[#333358] hover:border-[#c89b3c]/40 transition-colors">Set Primary</button>
                            )}
                            ...
                            <button onClick={() => removeLink(l.discord_id, l.rsn, l.primary_rsn)} className="text-xs text-[#7878a8] hover:text-white px-2 py-1 rounded bg-[#ED4245]/0 hover:bg-[#ED4245] border border-[#ED4245]/30 hover:border-[#ED4245]">Unlink</button>
                          </>
                        )}
                      </div>
                    </td>
          ...
          <div className="grid sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-[#1c1c36]">
            <div>
              ...
              {unlinkedDiscord.length === 0 ? (
                <p className="px-4 py-4 text-xs text-[#7878a8]">All Discord members are linked.</p>
              ) : (
                  ...
                  {unlinkedDiscord.map(d => (
                    <li key={d.discord_id} className="px-4 py-2.5 flex items-center justify-between gap-2">
                      ...
                      <button onClick={() => { setLinkDiscordId(d.discord_id); window.scrollTo({ top: 0, behavior: 'smooth' }) }}
                        className="text-xs text-[#7878a8] hover:text-[#7c5ce8] px-2 py-1 rounded border border-[#333358] hover:border-[#7c5ce8]/40 shrink-0">
                        Link →
                      </button>
                    </li>
                  ))}
            ...
            <div>
              ...
              {unlinkedIngame.length === 0 ? (
                <p className="px-4 py-4 text-xs text-[#7878a8]">All in-game names are linked.</p>
              ) : (
                <ul className="divide-y divide-[#1c1c36]">
