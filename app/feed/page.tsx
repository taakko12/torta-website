...
function tabCls(active: boolean, color = '#7c5ce8') {
  return active
    ...
    : 'px-4 py-2 rounded-lg text-sm font-medium bg-[#1c1c36] text-[#7070a0] hover:text-[#e8e8f0] transition-colors'
}
        ...
        {SECTIONS.map(s => (
          <Link key={s.key} href={`/feed?section=${s.key}`}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              section === s.key ? 'bg-[#7c5ce8] text-white' : 'bg-[#1c1c36] text-[#7070a0] hover:text-[#e8e8f0]'
            }`}>
            {s.label}
          </Link>
        ))}
...
function RecentDropFeed({ drops }: { drops: RecentDropItem[] }) {
  ...
  if (drops.length === 0)
    return <div className="rounded-xl border border-[#333358] bg-[#161628] py-12 text-center text-sm text-[#7070a0]">No drops recorded yet.</div>
        ...
        <div key={d.id} className="rounded-xl border border-[#333358] bg-[#161628] p-4 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {d.item_name && d.item_name !== 'Monthly aggregate' && (
              <span className="text-xs text-[#7070a0] ml-2">{d.item_name}</span>
            )}
            <div className="text-xs text-[#4a4a70] mt-0.5"><ClientDate iso={d.recorded_at} /></div>
          </div>
          <div className="text-right shrink-0">
            ...
            {d.discord_message_id && (
              <a href={`https://discord.com/channels/${GUILD_ID}/${DROPS_CHANNEL}/${d.discord_message_id}`}
                ...
                className="text-[10px] text-[#4a4a70] hover:text-[#c89b3c] transition-colors">↗ Discord</a>
            )}
          </div>
        </div>
      ))}
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <Link href="/feed?section=deaths&tab=monthly"
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${!isAlltime ? 'bg-[#cc5555] text-white' : 'bg-[#1c1c36] text-[#7070a0] hover:text-[#e8e8f0]'}`}>
            Monthly
          </Link>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {ACHIEVEMENT_TYPES.map(t => (
          <Link key={t.value}
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${(type ?? '') === t.value ? 'bg-[#c89b3c] text-[#0f0f1e]' : 'bg-[#1c1c36] text-[#7070a0] hover:text-[#e8e8f0]'}`}>
            {t.label}
          </Link>
        ))}
      </div>
