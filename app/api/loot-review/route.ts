...
export async function POST(req: Request) {
  const { dropId, label, reason, discordUrl } = await req.json()
  ...
  if (BOT_BASE_URL && BOT_ADMIN_SECRET) {
    fetch(`${BOT_BASE_URL}/api/loot-review-notify`, {
      ...
      body: JSON.stringify({ guildId: GUILD_ID, type: 'drop', label, reason: reason.trim(), requestedBy: 'Community', discordUrl }),
    }).catch(() => {})
  }
